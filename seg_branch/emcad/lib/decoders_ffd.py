...
class MSDC(nn.Module):
    def __init__(self, in_channels, kernel_sizes, stride, activation='relu6', dw_parallel=True):
        ...
        self.dwconvs = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(
                    self.in_channels//len(self.kernel_sizes), 
                    self.in_channels//len(self.kernel_sizes), 
                    kernel_size, stride, kernel_size // 2, groups=self.in_channels//len(self.kernel_sizes), bias=False),
                nn.BatchNorm2d(self.in_channels//len(self.kernel_sizes)),
                act_layer(self.activation, inplace=True)) for kernel_size in self.kernel_sizes
        ])
    ...
    def forward(self, x):
        ...
        for i, dwconv in enumerate(self.dwconvs):
            dw_out = dwconv(x[:,i*c//len(self.kernel_sizes):(i+1)*c//len(self.kernel_sizes)])
...
class CASCADE(nn.Module):
    def __init__(self, channels=[512,320,128,64], kernel_sizes=[1,3,5], expansion_factor=6, dw_parallel=True, add=True, lgag_ks=3, activation='relu6'):
        ...
        self.eucb3 = EUCB(in_channels=channels[0], out_channels=1, kernel_size=eucb_ks, stride=eucb_ks//2)
        self.eucb2 = EUCB(in_channels=channels[1], out_channels=1, kernel_size=eucb_ks, stride=eucb_ks//2)
        self.eucb1 = EUCB(in_channels=channels[2], out_channels=1, kernel_size=eucb_ks, stride=eucb_ks//2)
        ...
        self.up_c3 = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv2d(channels[0], channels[0], kernel_size=3, stride=1, padding=1, groups=channels[0], bias=False),
	        nn.BatchNorm2d(channels[0]),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[0], channels[1], kernel_size=1, stride=1, bias=True),
        )
        self.up_c2 = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv2d(channels[1], channels[1], kernel_size=3, stride=1, padding=1, groups=channels[1], bias=False),
	        nn.BatchNorm2d(channels[1]),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[1], channels[2], kernel_size=1, stride=1, bias=True),
        )
        self.up_c1 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[2], channels[3], kernel_size=1, stride=1, bias=True),
        )
